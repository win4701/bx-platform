...
def ensure_user(tg):
    c=db().cursor()
    r=c.execute("SELECT id FROM users WHERE tg_id=?",(tg,)).fetchone()
    if r: return r[0]
    t=int(time.time())
    c.execute("INSERT INTO users VALUES(NULL,?,?)",(tg,t))
    uid=c.lastrowid
    c.execute("INSERT INTO wallets VALUES(?,?,?,?)",(uid,0,0,0))
    c.execute("INSERT INTO subscriptions VALUES(?,?,?)",(uid,"silver",t))
    c.execute("INSERT INTO mining_state VALUES(?,?)",(uid,t))
    c.connection.commit()
    return uid
...
def state(uid:str):
    ...
    return {"wallet":{"bx":w[0],"usdt":w[1],"ton":w[2]},
            "subscription":tier,
            "mining":MINING[tier]}
...
def sell(uid:str,amount:float,against:str):
    if against not in("usdt","ton"): raise HTTPException(400)
    ...
    if sold+amount>DAILY_SELL_LIMIT: raise HTTPException(400,"LIMIT")
    price=0.72 if against=="usdt" else 0.95
    fee=amount*SELL_FEE; burn=amount*SELL_BURN
...
def casino(uid:str,game:str,bet:float):
    if game not in CASINO_GAMES: raise HTTPException(400)
    c=db().cursor(); u=ensure_user(uid)
...
def claim(uid:str):
    ...
    bx=h*MINING[tier]["bx"]; ton=h*MINING[tier]["ton"]
...
def deposit_address(provider:str,asset:str):
    if provider not in PROVIDERS: raise HTTPException(400)
    if asset not in("usdt","ton"): raise HTTPException(400)
    if provider=="ton":
...
def withdraw(uid:str,provider:str,asset:str,amount:float,address:str):
    ...
    if bal<amount: raise HTTPException(400)
    c.execute(f"UPDATE wallets SET {asset}={asset}-? WHERE user_id=?",(amount,u))
