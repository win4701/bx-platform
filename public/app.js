...
/* ===============================================================================================
   BX CONTRACT STON.FI - COMPLETE INTEGRATION
================================================================================================ */

// إعداد Ethers.js للاتصال بالعقد الذكي على شبكة TON
const provider = new ethers.providers.Web3Provider(window.ethereum);
const contractAddress = "EQCRYlkaR6GlssLRrQlBH3HOPJSMk_vzfAAyyuhnriX-7a_a"; // عنوان العقد الخاص بـ BX
const contractABI = [
...
/* ===============================================================================================
   Ston.fi API - للحصول على سعر التبادل بين BX و TON
================================================================================================ */

const STON_API_URL = "https://api.ston.fi/swap"; // تأكد من صحة الـ API URL

// استرجاع سعر التبادل بين BX و TON أو USDT
async function getSwapRate(fromToken, toToken) {
  try {
    const response = await fetch(`${STON_API_URL}/rate?from=${fromToken}&to=${toToken}`);
    if (!response.ok) {
      throw new Error('Error fetching swap rate');
    }
    const data = await response.json();
    return data.rate; // السعر بين العملات
  } catch (error) {
...
/* ===============================================================================================
   تنفيذ التبادل بين BX و TON/USDT عبر العقد الذكي
================================================================================================ */

async function performContractSwap(swapDetails) {
  try {
    const transaction = await contract.swapTokens(
      swapDetails.from,
      swapDetails.to,
      ethers.utils.parseUnits(swapDetails.amountFrom.toString(), 18)
    );

    console.log("Transaction sent:", transaction);

    // الانتظار حتى يتم تنفيذ المعاملة
    const receipt = await transaction.wait();
    console.log("Transaction confirmed:", receipt);

    return receipt;
  } catch (error) {
    console.error("Error in contract execution:", error);
    throw new Error('Transaction failed');
  }
}

/* ===============================================================================================
   دوال لشراء وبيع BX مقابل TON
================================================================================================ */

async function swapBXForTON(amountBX) {
  try {
    // الحصول على سعر التبادل بين BX و TON
    const rate = await getSwapRate("BX", "TON");

    // حساب المبلغ المطلوب بالعملة المستهدفة (TON)
    const amountInTON = amountBX * rate;

    // تنفيذ المعاملة عبر العقد الذكي
    const swapResult = await performContractSwap({
      from: "BX",
      to: "TON",
      amountFrom: amountBX,
      amountTo: amountInTON
    });

    alert(`Successfully swapped ${amountBX} BX for ${amountInTON} TON/USDT!`);
    // تحديث رصيد المستخدم بعد التبادل
    updateBalance(swapResult);
  } catch (error) {
    console.error("Error performing swapBXForTON:", error);
    alert("Error occurred during the swap");
  }
}

async function swapTONForBX(amountTON) {
  try {
    // الحصول على سعر التبادل بين TON و BX
    const rate = await getSwapRate("TON", "BX");

    // حساب المبلغ المطلوب بالعملة المستهدفة (BX)
    const amountInBX = amountTON * rate;

    // تنفيذ المعاملة عبر العقد الذكي
    const swapResult = await performContractSwap({
      from: "TON",
      to: "BX",
      amountFrom: amountTON,
      amountTo: amountInBX
    });

    alert(`Successfully swapped ${amountTON} TON for ${amountInBX} BX!`);
    // تحديث رصيد المستخدم بعد التبادل
    updateBalance(swapResult);
  } catch (error) {
    console.error("Error performing swapTONForBX:", error);
    alert("Error occurred during the swap");
  }
}

/* ===============================================================================================
   تحديث رصيد المحفظة بعد التبادل
================================================================================================ */

function updateBalance(swapResult) {
  // تعديل رصيد المحفظة بناءً على النتيجة
  WALLET.BX += swapResult.amountTo; // إضافة الرصيد المستلم
  renderWallet(); // إعادة رسم المحفظة بعد التحديث
}

/* ===============================================================================================
   واجهة المستخدم - التحكم في الأزرار
================================================================================================ */

async function performSwapBX() {
  const bxAmount = document.getElementById("bxAmount").value;
  if (bxAmount && bxAmount > 0) {
    await swapBXForTON(bxAmount);
  } else {
    alert("Please enter a valid BX amount.");
  }
}

async function performSwapTON() {
  const tonAmount = document.getElementById("tonAmount").value;
  if (tonAmount && tonAmount > 0) {
    await swapTONForBX(tonAmount);
  } else {
...
/* ===============================================================================================
   إضافة الأحداث لواجهة المستخدم (HTML)
================================================================================================ */

// إضافة الأحداث للأزرار في واجهة المستخدم
document.getElementById("swapButtonBX").addEventListener("click", performSwapBX);
document.getElementById("swapButtonTON").addEventListener("click", performSwapTON);
/* ================================================================================================
   PRICE ENGINE
================================================================================================ */
